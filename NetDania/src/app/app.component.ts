...
export class AppComponent implements OnInit, OnDestroy {
  ...
  forexData: ForexData[] = [
    ...
    // Declare forexData to hold the updated forex data
    private dataInterval: any;

    ngOnInit(): void {
      this.startDataUpdate(); // Start the interval to update data
    }
    ...
    private startDataUpdate(): void {
      // Update the forex data with new values every 5 seconds
    ...
    updateData(): void {
      // Simulate live updates by modifying bid, ask, change, and percentChange
      this.forexData = this.forexData.map(data => ({
        ...data,
        bid: +(data.bid + Math.random() * 0.01).toFixed(5),
        ask: +(data.ask + Math.random() * 0.01).toFixed(5),
        change: +(Math.random() * 2 - 1).toFixed(4),  // Convert to number
        percentChange: +(Math.random() * 2 - 1).toFixed(2),  // Convert to number
